...
use super::color::{ansi_bright_color, ansi_color, color_256};
    ...
    fn csi_dispatch(&mut self, params: &Params, intermediates: &[u8], _ignore: bool, c: char) {
        ...
        match c {
            ...
            'd' => {
                ...
                self.commands
                    .push(Command::MoveCursorToRow(n.saturating_sub(1) as usize));
            }
            'G' | '`' => {
                // CHA / HPA - Cursor Character/Horizontal Absolute
                let n = *params.iter().next().and_then(|p| p.first()).unwrap_or(&1);
                self.commands
                    .push(Command::MoveCursorToColumn(n.saturating_sub(1) as usize));
            }
            'E' => {
